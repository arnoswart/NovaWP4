...
covariates <- cbind(meanT = all.maps.extract.meas$Mean_temp_2011_2016.1, boars = all.maps.extract.meas$boars,denshunt = all.maps.extract.meas$Dens_hunt, indust = all.maps.extract.meas$Industrial.or.commercial.units) %>%
              as.data.frame() %>%
              filter(!is.na(meanT)) %>%
...
y_pos <- beta0 +beta1*covariates$meanT + beta2*covariates$boars + beta3*covariates$denshunt + beta4*covariates$indust + rnorm(nrow(covariates)) 

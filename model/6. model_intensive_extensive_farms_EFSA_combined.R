...
salm_status_centroids_combi.sf <- st_centroid(salm_status_combined.sf ) %>% 
  dplyr::select( status, frac_pos,type, geometry )
salm_status_buffers_combi.sf <- st_buffer(salm_status_centroids_combi.sf, dist=0.1 )
...
save.image( "./data/processed/all.maps.extracted_combi_buffer_d01.RData" )
load( "./data/processed/all.maps.extracted_combi_buffer_d01.RData" )

data.total_combi <- cbind( as.data.frame(salm_status_centroids_combi.sf) %>% dplyr::select( status, frac_pos,type),
                     all.maps.extract_combi %>% dplyr::select( -ID )) %>%
  na.omit()

p <- ggplot( data.total_combi  %>%
               pivot_longer(-c(status,frac_pos), "riskfactor" ) )
